    ...
    <div class="mx-4 mb-6 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div class="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between gap-3">
        <div class="flex items-center gap-2.5">
          ...
          <div>
            <p class="text-[13px] font-semibold text-slate-800 font-display leading-none">
              Ajustá la sugerencia
            </p>
            <p class="text-[11px] text-slate-400 mt-0.5 leading-none">
              Claude Haiku · lenguaje natural
            </p>
          </div>
        </div>
        ...
        @for (msg of messages(); track $index) {
          ...
          @if (msg.role === 'system') {
            <div class="flex items-start gap-2.5">
              <div
                class="w-5 h-5 rounded-md bg-[#003B73] flex items-center justify-center flex-shrink-0 mt-0.5"
              >
                <svg
              ...
              <div class="flex-1 min-w-0">
                <div
                  class="border-l-[2px] border-[#003B73]/30 pl-3 bg-slate-50 rounded-r-xl py-2.5 pr-3"
                >
          ...
          @if (msg.role === 'user') {
            <div class="flex justify-end gap-2 items-end">
              <div
                class="max-w-[75%] bg-[#003B73] text-white px-4 py-2.5 rounded-2xl rounded-br-md shadow-sm"
              >
                <p class="text-sm leading-relaxed">{{ msg.content }}</p>
              </div>
              <div
                class="w-5 h-5 rounded-md bg-slate-200 flex items-center justify-center flex-shrink-0"
              >
                <svg
                  class="w-2.5 h-2.5 text-slate-500"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2.5"
      ...
      <div class="px-4 pb-4 pt-1 border-t border-slate-100">
        <div class="relative flex items-center">
          ...
          >
            <svg
              class="w-3 h-3 text-white disabled:text-slate-400"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              stroke-width="2.5"
